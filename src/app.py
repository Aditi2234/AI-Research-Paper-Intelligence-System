...
with st.sidebar:
    ...
    show_entities = st.checkbox("Extract named entities (NER)", value=True)
    st.markdown("---")
    st.markdown(
        "**How it works**\n\n"
        ...
        "5. spaCy NER extracts named entities (orgs, dates, people, etc.)"
    )
    ...
    for r in results:
        with st.container(border=True):
            ...
            if show_entities and "entities" in r:
                if r["entities"]:
                    ent_tags = "  ".join(
                        f"`{e['text']}` _{e['label']}_" for e in r["entities"]
                    )
                    st.markdown(f"**🧩 Named Entities:** {ent_tags}")
                else:
                    st.markdown("**🧩 Named Entities:** _None found_")
